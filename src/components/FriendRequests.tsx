    ...
    <>
      {friendRequests.length === 0 ? (
        ...
        friendRequests.map((request) => (
          <div key={request.senderId} className='flex gap-4 items-center bg-card rounded-lg px-4 py-2 border shadow-sm'>
            <UserPlus className='text-current' />
            <p className='font-medium text-lg text-card-foreground'>{request.senderEmail}</p>
